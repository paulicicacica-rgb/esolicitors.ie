    ...
    async function send() {
      ...
      try {
        ...
        if (stage === "done" && collected.phone) {
          ...
          // Extract legal issue from first user message
          const firstUserMsg = history.find(m => m.role === "user");
          const issue = firstUserMsg ? firstUserMsg.content.substring(0, 120) : "";

          // POST lead to Google Sheets
          fetch("/api/leads", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
